...
_Note: Changing to the game directory is technically not necessary, but running the command prompt as an administrator can be risky if you don't know what you're doing. So, by changing to the game directory first, if anything goes wrong then it's only the game files which are affected, and these can be reset thoguh Steam (with the "validate local files" option)._
